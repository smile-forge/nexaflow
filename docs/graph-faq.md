...
`nfi graph stats` — **62,419 nodes, 121,278 edges, 16,101 hyperedges** over 5,613 files.
...
| area | not understood | what they are |
|---|---|---|
| **`src/` (our code)** | **8** | two `.ndjson` fixtures, a font licence, an `.rtf`, an `.ico`/`.otf`, a `.config` |
| repo root / tooling | 59 | images and docs, five `.ps1`, the CI `.yml`, git dotfiles |
| `external/` (submodules) | 1,002 | third-party bindings, CI, queries and docs in other languages |

**The build and packaging gap is closed.** It used to be the honest answer to "what is missing here" — the
WiX installer and the MSBuild logic every project inherits were both unread. `.wxs`/`.wxl`/`.props`/
`.targets`/`.manifest` are XML and now read with the `xml` grammar; `.wixproj` always went through the
csproj path, being a project file with `PropertyGroup`s like any other. What still needs a grammar the
binding does not ship: `.ps1` and `.yml`.

### Generated files, and why the answer was not "skip C"

`external/` holds 220 `.c`/`.h` files — the tree-sitter grammars themselves — and the tempting response was
to leave `.c` unregistered, since the C# grammar's `parser.c` alone is 31 MB of generated tables that nobody
will ever query. That reasoning is wrong in a way worth recording: **it throws out an entire language
because one dependency ships generated files in it.** The hand-written `scanner.c` sitting in the same
folder is real source, and someone debugging a grammar would reasonably ask about it.

So the test is the file, not the language. `RepoFiles.IsGenerated` reads the head of a file and looks for
the marker every generator leaves (`@generated`, `<auto-generated`, `DO NOT EDIT`). A generated file is
**located, sized, labelled with its language and left unexpanded** — it gets `generated: true` and
contributes no types, members or edges. Its hand-written neighbour is parsed in full. A file too large to
read now says which it was, via `unread: generated` or `unread: too-large-or-binary`, rather than being
silently emptier than its neighbours.
...
| Which parts are app code vs tests/tooling/generated? | Product tree + path convention (`src/Nexaflow.Tests/**`), or `sdk` on the project node: a test project declares `MSTest.Sdk`, and that is often the ONLY statement it makes (checked against an unrelated WPF app whose test project carries no package reference at all, so a "depends_on MSTest" rule would have missed it). **Generated** code is the interesting one: `annotated` hyperedges name `[ObservableProperty]`/`[RelayCommand]`, so the generated public surface is identifiable from its declaration | 🟡 |
