...
> 2. **`ModalCard` + a ratchet (S).** ✅ 2026-09-11, with Product's take-snapshot form as the first consumer. A lookless `HeaderedContentControl` in `Visuals.Common` (scrim +
...
> | `ProductView` settings · concern snaplinks · restructure (snapshot moved in PR 2) | forms | `ModalCard` — PR 3 |
...
> | `RibbonEditor` (Core) — missed here, found by the ratchet's first run | whole-window form | `ModalCard` — PR 3, or with §D3's rewrite |
>
