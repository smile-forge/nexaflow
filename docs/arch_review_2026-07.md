...
| E1 | Hand-rolled modal overlays — **14 in 7 files** — and the confirm state machine written **4 times** | **re-audited 2026-09-11: open, and the old row was wrong.** Its "13 sites" counted graph nodes that *mention* `ConfirmationRequest`; adopters were **one**. Questions spread through the shell instead (~30 call sites); forms had nothing to move onto. Inventory and a three-PR plan in §E1 | `$nfi graph grep 'Background="#CC[0-9A-Fa-f]{6}"' --mode content`, keeping each hit whose child is a centred card (drop the shell's own host, FileSystem's tick and drop tooltip, Text's banner) — §E1's PR 2 replaces this with a ratchet list whose line count *is* the answer | M | P2 |
...
> **Three PRs:**
>
> 1. **Questions → shell (S).** Registry's, Projects' and ProductManager's deletes `await ConfirmAsync(…,
