...
| `…/<Type>/<Type>Diagram.cs` (or `Chart`) | Only where a diagram still has a model: the tree read back into what it describes. A diagram without one has its stages write what they work out into its own nodes (`…/<Type>/<Type>Nodes.cs`, internal, as `PieNodes` and `RadarNodes`) — and one with nothing to work out has none | `VennDiagram` |
