...
| read the `classDef`, `class` and `style` lines, and work out what everything is styled with | `MermaidStyling`, given the diagram's own roles: `Defined`, `Applied`, `Styled`; from a stage, `Resolve` for what is missing and `Style` for what each thing is drawn with, hung on the name first writing it as a `StyledNode` the builder reads with `StyleOf` — keyed by what something stands for, through `idOf`, where that is not the name written, as a state diagram's `[*]` is the dot its scope starts or stops at (a diagram still with a model: `Styles` there) |
| draw only part of a graph-shaped diagram, as a `config: nexaflow:` block asks | the builder hands `Fold` its nodes and links (`DiagramChart`) before placing anything, then asks `Draws`, and draws chips with `Chipped` and leftovers with `Spilled` |
...
| set words that wrap to a width, breaking where a `<br>` or a `\n` says to, each line typed into as the characters it holds | `Wrapped` — a line to each break, standing for the pieces between (`PartRun`), and a line wider than the width broken by the type engine into lines set in the middle of one another, one run a press still lands in letter by letter (`LayoutWords.IndexAt`). `Written` sets a break as the lines it makes, pressed rather than typed into |
| lay a tree out tidily — children beside their parent, the root's either side | `DiagramTree.Lay` |
| lay nodes joined by lines out in ranks — a flowchart, a state chart | `DiagramLayers.Lay` of `DiagramCell`s and `DiagramJoin`s: ranks by how far the links reach, an order that keeps few lines crossing (sweeps by the average and the middle of what each place joins, neighbours swapped where that takes a crossing away, the fewest crossings found kept), each place set across its rank by `DiagramAlignment` — Brandes and Köpf's, so a long line runs straight and a run of cells beside it still stands in a line — boxes laid out in their own space and run their own way, and a route for every line — turning in the air between two ranks rather than between the middles of what it joins, running alongside a rank it passes rather than across what is in it, and bowed aside from any other line joining the same pair |
...
| set words that may hold an entity code or a binding — drawn as what it says, and so pressed rather than typed into | `Wrapped` and `Written` on the builder: what each piece of the words is — its own characters, a code, a break, a binding — was found by `WithWordPieces` when the block was read and is held on the diagram (`MermaidWords`), so the builder sets the pieces and never looks at the characters |
...
| name an icon, however the diagram writes one | read it as a `MermaidKinds.Icon` round what was written — `MermaidOutline.Decoration` does for `::icon(…)`, and `Properties` for an `icon:` key; the pipeline's `WithIcons` says which Fluent UI icon it is (`MermaidIcons`, a `RenderedIconNode` holding its font and glyph), and the builder draws it with `Iconed`, saying itself what is drawn where the app has no such icon |
