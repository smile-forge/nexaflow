...
/// <summary>
/// Attaches and detaches a disk image through the Windows Virtual Disk API (virtdisk.dll) — the same service
/// the <c>Mount-DiskImage</c>/<c>Dismount-DiskImage</c> cmdlets wrap, but called directly so the elevated
/// bridge never spawns a process. Runs elevated here because VHD/VHDX attach needs administrator rights.
/// <para>
/// This file is a verbatim copy of the feature's <c>Services/NativeVirtualDisk.cs</c>. The trust boundary
/// forbids sharing code (the bridge references only Elevation.Contracts), so — as with the scripts it
/// replaces — the two copies are kept identical.
/// </para>
/// </summary>
