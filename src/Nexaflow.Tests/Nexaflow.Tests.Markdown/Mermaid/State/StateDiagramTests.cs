...
public class StateDiagramTests
{
    ...
    public void ACompositeStateStandsForTheWholeOfWhatItWasWrittenAs()
    {
        ...
        var read = StateDiagram.Read(source);
    ...
    {
        Assert.IsTrue(StateDiagram.Read("stateDiagram-v2\n  hide empty description\n  one").Plain);
        Assert.IsFalse(StateDiagram.Read("stateDiagram-v2\n  one").Plain);
    }

    [TestMethod]
    public void TheFrontMatterIsApplied()
    {
        var read = StateDiagram.Read("---\nconfig:\n  state:\n    nodeSpacing: 40\n    wrappingWidth: 90\n    forkWidth: 60\n"
                                   + "---\nstateDiagram-v2\n  one --> two");
