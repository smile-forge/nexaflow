...
public class BlockDiagramTests
{
    ...
    {
        var diagram = BlockDiagram.Read("block-beta\n  columns 3\n  a[\"A label\"] b:2 c:2 d");
    ...
    {
        Assert.IsNull(BlockDiagram.Read("block-beta\n  a b c").Columns, "nothing written lays them all on one row");
        Assert.IsNull(BlockDiagram.Read("block-beta\n  columns auto\n  a b c").Columns, "and so does auto");
        Assert.AreEqual(1, BlockDiagram.Read("block-beta\n  columns 1\n  a b c").Columns);
    }
    ...
    public void ABlockArrowPointsEveryWayItSays_AndRightWhereItSaysNothing()
    {
        ...
        var arrow = BlockDiagram.Read("block-beta\n  a<[\"Label\"]>(right)").Items[0];
    ...
    {
        Assert.AreEqual(BlockConfig.Air, BlockDiagram.Read("block-beta\n  a").Config.Padding);
        Assert.AreEqual(12, BlockDiagram.Read("---\nconfig:\n  block:\n    padding: 12\n---\nblock-beta\n  a").Config.Padding);
    }

    [TestMethod]
    public void TheDocumentationsOpeningDiagramIsReadWhole()
    {
        var diagram = BlockDiagram.Read(BlockGrammarTests.Intro);
        ...
        Assert.AreEqual("Where it runs", BlockDiagram.Read("---\ntitle: Where it runs\n---\nblock-beta\n  a").Block.Title?.Text);
        Assert.AreEqual(3, BlockDiagram.Read("block-beta\n  title Where it runs").Items.Count(item => item.Id.Length > 0 && item.Id != "title"));
    }

    private static MermaidShape Shaped(string source) => BlockDiagram.Read(source).Items[0].Shape;

    private static BlockTowards Pointing(string source) => BlockDiagram.Read(source).Items[0].Towards;
}
