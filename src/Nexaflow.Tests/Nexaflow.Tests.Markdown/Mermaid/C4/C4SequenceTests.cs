...
public class C4SequenceTests
{
    ...
    {
        var diagram = C4Sequence.Of(MermaidStaged.Read("C4Sequence\nContainer(spa, \"Single-Page App\", \"Angular\", \"Delivers it\")\n"
    ...
    public void ADescriptionIsDrawnOnlyWhereItIsAskedFor()
    {
        ...
        Assert.IsNull(C4Sequence.Of(MermaidStaged.Read(source)).Find("p")?.Card?.Said);
        Assert.AreEqual("Somebody",
                        C4Sequence.Of(MermaidStaged.Read("C4Sequence\nSHOW_ELEMENT_DESCRIPTIONS()\nPerson(p, \"P\", \"Somebody\")\nRel(p, p, \"x\")"))
    ...
    {
        var message = C4Sequence.Of(MermaidStaged.Read("C4Sequence\nPerson(a, \"A\")\nSystem(b, \"B\")\nRel(a, b, \"Visits\", \"HTTPS\", \"Every day\")"))
                                .Messages.Single();
    ...
    {
        Assert.IsNull(C4Sequence.Of(MermaidStaged.Read("C4Sequence\nPerson(a, \"A\")\nRel(a, a, \"x\")")).Messages.Single().Number);

        var numbered = C4Sequence.Of(MermaidStaged.Read("C4Sequence\nSHOW_INDEX()\nPerson(a, \"A\")\nRel(a, a, \"x\")\nRel(a, a, \"y\")"))
                                 .Messages.ToList();
    ...
    {
        var numbers = C4Sequence.Of(MermaidStaged.Read("C4Sequence\nSHOW_INDEX()\nPerson(a, \"A\")\nRel(a, a, \"one\", $index=Index())\n"
    ...
    {
        Assert.IsTrue(C4Sequence.Of(MermaidStaged.Read("C4Sequence\nPerson(a, \"A\")\nRel(a, a, \"x\")")).Config.Mirrored);
        Assert.IsFalse(C4Sequence.Of(MermaidStaged.Read("C4Sequence\nSHOW_FOOT_BOXES(false)\nPerson(a, \"A\")\nRel(a, a, \"x\")")).Config.Mirrored);
    }
