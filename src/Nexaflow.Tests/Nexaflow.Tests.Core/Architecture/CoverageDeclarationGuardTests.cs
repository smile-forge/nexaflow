...
namespace Nexaflow.Tests.Core.Architecture;

/// <summary>
/// Every Core <c>[TestClass]</c> must declare the product node it backs with <c>[CoversNode]</c> or opt out
/// with <c>[NoCoverage]</c> — the CI backstop to the author-time analyzer. Id validity is additionally
/// checked wherever <c>.product/tree.json</c> is present (skipped on a clean checkout without it).
/// </summary>
[TestClass]
