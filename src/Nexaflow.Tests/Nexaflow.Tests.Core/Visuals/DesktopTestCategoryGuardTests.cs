...
public class DesktopTestCategoryGuardTests
{
    ...
    {
        var root = Path.Combine(SuiteRoot(), "src", "Nexaflow.Tests", "Nexaflow.Tests.Core");
        Assert.IsTrue(Directory.Exists(root), $"could not find the suite's own source at {root}");
