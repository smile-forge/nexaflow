...
public sealed class ResolveGraph(string main) : IAstStage
{
    ...
    public ContentNode Run(ContentNode tree)
    {
        ...
        var branches = new List<string>(StringComparer.Ordinal.Equals(main, string.Empty) ? [] : [main]);
        ...
        foreach (var line in tree.SelfAndDescendants().Where(node => node.Kind == MermaidKinds.Line))
        {
            ...
            switch (stated.Kind)
            {
                case GitKinds.Branch:
                    if (Named(stated) is not { Length: > 0 } made) break;

                    if (branches.Contains(made, StringComparer.Ordinal)) wrong[stated] = $"A branch is made once, and {made} is already one.";
                    else branches.Add(made);
                ...
                case GitKinds.Merge:
                    ...
                    if (Named(stated) is not { Length: > 0 } merged) break;
                ...
                case GitKinds.Pick:
                    ...
                    if (Option(stated, "id") is not { Length: > 0 } taken)
                        wrong[stated] = "A cherry-pick names the commit it takes: cherry-pick id: \"Alpha\".";
                    else if (!ids.Contains(taken, StringComparer.Ordinal))
                        wrong[stated] = $"No commit above this has the id {taken}.";
