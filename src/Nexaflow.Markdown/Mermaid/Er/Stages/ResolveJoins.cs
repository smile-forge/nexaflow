...
/// written on — every subgraph is read first, and then every relationship is told what its ends name.
/// </summary>
public sealed class ResolveJoins : IAstStage
{
    ...
    public ContentNode Run(ContentNode tree)
    {
        ...
        foreach (var line in tree.SelfAndDescendants().Where(node => node.Kind == MermaidKinds.Line))
        {
            ...
            if (stated.Kind == ErKinds.Subgraph && stated.Said(ErRoles.Opened) is { } key
                && Said(stated, ErRoles.Space) is { Length: > 0 } name)
                boxes.TryAdd(name, key);
        }

        if (boxes.Count == 0) return tree;

        var joined = new Dictionary<ContentNode, string>();

        foreach (var stated in relations)
            foreach (var named in stated.SelfAndDescendants().Where(node => node.Kind == ErKinds.Named))
                if (Said(named, ErRoles.Id) is { Length: > 0 } name && boxes.TryGetValue(name, out var key))
                    joined[named] = key;

        if (joined.Count == 0) return tree;

        return AstRewrite.Each(tree, node => joined.TryGetValue(node, out var key)
            ? node.Saying(ErKinds.Fact, ErRoles.Boxed, key)
            : node);
    }
