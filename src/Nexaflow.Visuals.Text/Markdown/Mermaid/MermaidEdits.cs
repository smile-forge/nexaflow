...
/// <summary>
/// What an edit means in a diagram: ending a line, taking back a character, typing what a place cannot hold, and
/// renaming a name where it is declared.
///
/// <para>
...
/// and its value still to fill in; a Venn diagram's next item under an item or a set, and its next set anywhere else. Never
/// through the middle of what the caret is on, which would leave half a label on each line. A diagram with no shape of
/// line to start from, or one shown as the characters it was written with, is text for as long as it is, and Enter starts
/// a line in it indented like the one it was pressed on.
/// </para>
/// <para>
...
/// </para>
/// <para>
/// Told in the document's offsets, which are the ones everything the diagram drew is named in; what reads the diagram's
/// own source from the top — its grammar, its lines — reads <see cref="ContentEdit.Local"/>.
/// </para>
/// </summary>
internal sealed class MermaidEdits : IOnEdit
{
    public static MermaidEdits Instance { get; } = new();
    ...
    public EditState? Typing(ContentEdit edit, string text)
    {
        ...
        if (part is null || MermaidDiagrams.Grammar(MermaidBlock.Read(edit.Source).Diagram)?.Escaping(part, caret, text) is not { } writing)
    ...
    public EditState? Settling(ContentEdit edit, string separator)
    {
        ...
        if (Started(edit) is { } started) return started;

        var local = edit.Local;
        var line = Line(local.Source, local.Caret);

        return edit.Back(local.Write(Newline(local.Source) + Leading(local.Source, line.Start, line.End)));
    }
    ...
    public EditState? Erasing(ContentEdit edit, bool forward)
    {
        ...
        if (edit.Landing.Laid.Holes.Any(hole => hole.Sits().Start == caret))
        {
            var local = edit.Local;
            var source = local.Source;
            var line = Line(source, local.Caret);
                ...
                && source[line.Start..line.End].Trim() == blank.Text.Trim())
            {
                ...
                return edit.Back(local.Remove(from, line.End - from));
            }
        ...
        return edit.Landing.Laid.Root.WordsAt(caret) is { Part: { } part } && Inside(edit, part) && caret == (forward ? part.End() : part.Start)
            ? state
            : null;
    }
    ...
    /// than guessed at.
    /// </remarks>
    public EditState Edited(ContentEdit before, EditState after)
    {
        var then = before.After(after);
        var local = then.Local;
        var renamed = Renamed(before.Source, local);

        return ReferenceEquals(renamed, local) ? after : then.Back(renamed);
    }

    /// <summary>What a rename made in <paramref name="after"/> comes to, counted from the top of the diagram — or <paramref name="after"/> itself where it was no rename.</summary>
    private static EditState Renamed(string before, EditState after)
    {
        var now = MermaidBlock.Read(after.Source);
        if (MermaidDiagrams.Grammar(now.Diagram) is not { } grammar) return after;

        var (start, end, written) = Changed(before, after.Source);

        var was = grammar.Names(MermaidBlock.Read(before).Reading.Root);
    ...
    /// </summary>
    private static EditState? Started(ContentEdit edit)
    {
        if (Drawn(edit).Any(piece => piece.Kind == LayoutText.SourceKind)) return null;

        var state = edit.Local;
        ...
        return edit.Back((state with { Raw = null })
            .MoveCaretTo(on.End)
            .Write(newline + indent + blank.Text)
            .MoveCaretTo(on.End + newline.Length + indent.Length + blank.Caret));
    }

    /// <summary>What was drawn of this diagram, and nothing of what surrounds it.</summary>
    private static IEnumerable<Piece> Drawn(ContentEdit edit) =>
        edit.Landing.Laid.Root.SelfAndDescendants().Where(piece => piece.Part is { } part && Inside(edit, part));

    /// <summary>Whether a part is one of this diagram's, rather than of the document it is written in.</summary>
    private static bool Inside(ContentEdit edit, ISourcePart part) => part.Start >= edit.Start && part.End() <= edit.End;
    ...
    {
        var laid = edit.Landing.Laid;

        if (laid.Holes.FirstOrDefault(hole => hole.Sits().Start == caret) is { Exists: true, Part: ContentPart hole } && Inside(edit, hole))
            return hole;

        return laid.Root.WordsAt(caret).Part is ContentPart words && Inside(edit, words) ? words : null;
    }
