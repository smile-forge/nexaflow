...
internal sealed class XyBuilder : MermaidBuilder<XyChart>
{
    ...
    private XyBuilder(EditState state, MarkdownPalette palette, double pixelsPerDip, double room, bool writing)
    ...
    public static Laid Build(EditState state, MarkdownPalette palette, double pixelsPerDip, double room = double.PositiveInfinity,
                             bool writing = false) =>
        new XyBuilder(state, palette, pixelsPerDip, room, writing).Lay();
